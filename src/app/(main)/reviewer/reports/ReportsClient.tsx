...
import { Search, ChevronDown, FileText, MapPin } from "lucide-react";
import { fmtDateTime } from "@/lib/format";
import type {
  ReviewerReportRow,
  ReportStatusKey,
} from "@/lib/reviewer/get-reviewer-reports";
...
function statusStyle(s: ReportStatusKey) {
  switch (s) {
    case "SUBMITTED":
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#627D98]">
            All citizen reports in the system.
          </p>
        </div>
        <span className="mt-2 text-xs font-semibold text-[#627D98]">
          {filtered.length} of {rows.length} · {total} total
        </span>
      </div>
